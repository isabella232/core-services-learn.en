...
description: Learn how to get your Experience Platform Launch property's embed codes and implement them in your website. This lesson is part of the Implementing the Experience Cloud in Mobile iOS Objective-C Applications tutorial.
seo-description:
...
The Installation Instructions for mobile Experience Platform Launch properties are a collection of code snippets that you either run in your Terminal or add to specific locations in your mobile app.
...
If you have previously used Experience Platform Launch in websites, one of the first things you will notice is that there is a lot more information in this modal than for web properties.
...
1. In the Experience Platform Launch interface, change the operating system to `iOS`

1. Copy the first iOS instruction `pod init`, by clicking the ![Copy](images/mobile-launch-copyIcon.png) icon

   ![Copy pod init to your clipboard in the Experience Platform Launch interface](images/ios/mobile-launch-install-copyPodInit.png)
...
1. In the Experience Platform Launch interface, copy the list of dependencies by clicking the ![Copy](images/mobile-launch-copyIcon.png) icon. Note how there is a line corresponding to each one of the extensions you added in the earlier lesson. Each extension has its own set of code which builds on the Mobile Core extension and can only be added or removed with an app update:
   ...
   ![Copy dependencies to your clipboard in the Experience Platform Launch interface](images/ios/mobile-launch-install-copyDependencies.png)
...
1. In the Experience Platform Launch interface, copy the next iOS instruction `pod repo update`, by clicking the ![Copy](images/mobile-launch-copyIcon.png) icon
...
1. In the Experience Platform Launch interface, copy the next iOS instruction `pod install`, by clicking the ![Copy](images/mobile-launch-copyIcon.png) icon

   ![Copy pod install to your clipboard in the Experience Platform Launch interface](images/ios/mobile-launch-install-copyPodInstall.png)
...
1. In the Experience Platform Launch interface, scroll to the **[!UICONTROL Add Initialization Code]** section and choose **[!UICONTROL Objective C]** as the iOS language you are using.
...
1. In the Experience Platform Launch interface, copy the two lines related to the Core extension, by clicking the second ![Copy](images/mobile-launch-copyIcon.png) icon in the **[!UICONTROL Add Initialization Code]** section. The first line turns on console logging statements (available options are "debug", "verbose", "warning", and "error"). The second line points to the unique identifier of the Experience Platform Launch environment. This is important, as you will need to update this value when we are ready to deploy the app to the production environment.
...
1. **Calls to retrieve the Experience Platform Launch configuration** (filter your console to `adobedtm.com`). Note the extension configurations which you entered in the earlier lesson. While adding the extension requires an update to the app, these settings can be managed externally in Experience Platform Launch and changed at any time:
